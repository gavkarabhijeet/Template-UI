...
export class Mapping3Component implements OnInit {
  ...
  disabledMapping3Check: boolean = true;
  disabledMapping3CheckSubmit:boolean =true;
    ...
    this.mappingService.getProjectData(this.projectId).then((data) => {
      ...
      this.mappingService.getMappingData(this.projectId).then((getMappingDataResult) => {
        ...
        if (getMappingDataResult.message == "Data not found.") {
          ...
          this.disabledMapping3Check = true;
          this.disabledMapping3CheckSubmit=true;
          ...
          this.mappingService.getProjectData(this.projectId).then((data) => {
            ...
            if (data[0].event == "recording_test_results_TXN_REV-UAT" || data[0].event == "recording_test_results-UAT" || data[0].event == "recording_test_results_TXN_REV-SIT" || data[0].event == "recording_test_results-SIT" || data[0].status === "Ready for Production Request Initiated" || data[0].status === "Ready for Production Verified" || data[0].status == "SUCCESS") {
              ...
              var data1=[];
              var data2=[];
              data1= getMappingDataResult[0].mappingObj.mapping3RequestObject1;
              data2 = getMappingDataResult[0].mappingObj.mapping3ResponseObject1;
            ...
            if(value1 == "not empty" && value2 == "not empty"){
              this.disabledMapping3Check=false;
              ...
              this.disabledMapping3CheckSubmit = false;
  ...
  getClientFileService(projectId) {
    ...
    this.mappingService.getFileDataByProjectId(projectId,type).then(async (data) => {
      ...
      if (data.length >= 1) {
        ...
        console.log("this.fileData", this.fileDataByProjectId);
  ...
  */
  ...
  deleteValueRequest(data) {
    console.log("data 1234567890 =====>", data)
...
*/
  ...
  */
  ...
  getFlattenStructure1(requestData, responseData, clientUrl) {
    ...
    this.mappingService.postESQL(finalEsqlObject).then((dataEsql) => {
      ...
      // this.toastr.success("Response for ESQL Request 1 :" + dataEsql.message);
  ...
  */
  ...
  getFlattenStructureResponse1(requestData, responseData, clientUrl) {
    ...
    this.mappingService.postESQL(finalEsqlObject1).then(async (dataEsql) => {

      console.log("response dataEsql = ", dataEsql)

      //  this.toastr.success("Response for ESQL Response 2 :" + dataEsql.message);
      this.errmsg = dataEsql.message;
      $("#e").css("display", "block").delay(2000).fadeOut(400);

      this.mappingService.postESQL(finalEsqlObject2).then(async (dataEsql) => {

        console.log("response dataEsql = ", dataEsql)

        // this.toastr.success("Response for ESQL Response 3:" + dataEsql.message);
        this.errmsg = dataEsql.message;

        $("#f").css("display", "block").delay(2000).fadeOut(400);

        this.mappingService.postESQL(finalEsqlObject3).then((dataEsql) => {

          console.log("response dataEsql3 = ", dataEsql)

          // this.toastr.success("Response for ESQL Response 4:" + dataEsql.message);
          this.errmsg = dataEsql.message;

          $("#g").css("display", "block").delay(2000).fadeOut(400);
          ...
          console.log("Final Yaml Creation output ");
          // this.isLoading=false
          ...
          if (err) {
            ...
            $("#h").css("display", "block").delay(2000).fadeOut(400);
            // this.toastr.error("ESQL4 Response.Please Try Again.." + err.message);
        ...
        if (err) {
          // this.toastr.error("ESQL3 Response.Please Try Again.." + err.message);
          this.errmsg = err.message;

          $("#i").css("display", "block").delay(2000).fadeOut(400);
          console.log("PRINT EXCEPTION ", err);
          this.isLoading = false;
        }
      })
      ...
      if (err) {
        ...
        $("#j").css("display", "block").delay(2000).fadeOut(400);
        console.log("PRINT EXCEPTION in ", err);
        this.isLoading = false;
      }
    })
  ...
  async yamlCreation(requestD, responseD, clientUrl) {
    ...
    if (this.arrayFlagSubmitResponse == true) {
      ...
      console.log("finalYaml in if= ", JSON.stringify(finalYaml[0]))
      console.log("yamlObject in if = ", JSON.stringify(yamlObject))
      console.log("request Array",[requestField['fields']]);
      localStorage.setItem("yamlFields",JSON.stringify([requestField['fields']]));

      this.mappingService.postYamlData(yamlObject).then(async (yamlResponse) => {

        console.log("yamlResponse = ", yamlResponse)
        // this.toastr.success("Response for Yaml :" + yamlResponse.message);
        this.errmsg = yamlResponse.message;

        $("#k").css("display", "block").delay(2000).fadeOut(400);
        ...
        // mappingDataObject["mappingObj"] = mapping3RequestDataObject;
        // mappingDataObject["projectId"] = localStorage.getItem('projectId')
        // await this.mappingService.postSaveMappingData(mappingDataObject).then((postSaveMappingDataResponse) => {
        //   console.log("postSaveMappingDataResponse  =====>", postSaveMappingDataResponse);
        //   this.spinner.hide();
        // })
      }).catch(err => {
        console.log("PRINT : PROBLEl WITH YAM 1", err);
        $("#l").css("display", "block").delay(1500).fadeOut(200);
        setTimeout(() => {
          $("#m").css("display", "block").delay(1500).fadeOut(200)
        }, 1501);
      ...
      console.log("finalYaml2 else = ", JSON.stringify(finalYaml2[0]))
      console.log("yamlObject2 else = ", JSON.stringify(yamlObject2))

      this.mappingService.postYamlData(yamlObject2).then(async (yamlResponse) => {

        console.log("yamlResponse = ", yamlResponse)
        // this.toastr.success("Response for Yaml :" + yamlResponse.message);
        this.errmsg = yamlResponse.message;

        $("#n").css("display", "block").delay(2000).fadeOut(400);
        if (this.alreadyDataPresent == false) {
          var mappingDataObject = {};
          var mapping3RequestDataObject = {
            "mapping3RequestObject1": this.mapping3RequestObject1,
        ...
        // mappingDataObject["mappingObj"] = mapping3RequestDataObject;
        // mappingDataObject["projectId"] = localStorage.getItem('projectId')
        // await this.mappingService.postSaveMappingData(mappingDataObject).then(async (postSaveMappingDataResponse) => {
        //   console.log("postSaveMappingDataResponse  =====>", postSaveMappingDataResponse);
  ...
  saveDataOfMapping() {
    if (this.alreadyDataPresent == false) {
      var mappingDataObject = {};
      var mapping3RequestDataObject = {
        "mapping3RequestObject1": this.requestData,
        "mapping3ResponseObject1": this.responseData,
      };

...
@Component({
  selector: 'app-mapping7',
  templateUrl: './mapping7.component.html',
  styleUrls: ['./mapping7.component.css'],
  providers: [mapping2Service]
})
export class Mapping7Component implements OnInit {
  regVan = /^[a-zA-Z0-9]+$/;
  source = [];
  requestData = [];
  requestDataSource = [];
  ...
  extractedDataSource3;
  combinedDataAfterExtractionSource2 = [];
  combinedDataAfterExtractionSource3 = [];
  ...
  // yaml transaction reversal
  requestProperty3 = [];
  requestDataFiltered3 = [];
  requestLayout3 = [];
  requestPropertyNonArray3 = [];
  requestExtraction3 = [];
  requestPropNonArray33 = [];
  finalRequestWithoutArray3 = [];
  finalRequest1233 = [];
  finalRequest2233 = []
  ...
  dataTypeICICIRequest3;
  descriptionICICIRequest2;
  descriptionICICIRequest3;
  dataTypeClientRequest2;
  dataTypeClientRequest3;
  dataTypeClientResponse2;
  dataTypeClientResponse3;
  ...
  disabledMapping7Check:boolean = true;
  ansName: boolean;
  vanNumber='';
  retryRule: string;
  ...
  constructor(private makerService: MakerService, private mapping4Service: mapping2Service, private modalService: NgbModal,
    ...
    checkData(){
      ...
      var reqData3=this.requiredFieldCheckreq3();
      var reqData2=this.requiredFieldCheckreq2();
      console.log("Data for check",reqData,resData,reqData2);
     if(reqData == true && resData == true && reqData2 == true && resData2 == true && reqData3 == true){
      this.disabledMapping7Check=false;
     }
     else{
      this.disabledMapping7Check=true;
     }
    }
    requiredFieldCheckreq() {
      ...
      requiredFieldCheckres() {
        console.log("function called");
        console.log("this.requestData--->",this.responseData);
        var nonempty=[];
        ...
        requiredFieldCheckres2() {
          console.log("function called");
          console.log("this.requestData--->",this.responseData2);
          var nonempty=[];
          ...
          requiredFieldCheckreq2() {
            ...
            requiredFieldCheckreq3() {
              console.log("function called");
              console.log("this.requestData--->",this.requestData3);
              var nonempty=[];
              for (var i = 0; i < this.requestData3.length; i++) {
                if(this.requestData3[i].directRowNo !=""){
                  nonempty.push(this.requestData3[i]);
                  console.log
                  ( nonempty
                  );
                }
  ...
  ngOnInit() {
    ...
    console.log("-----------------4A TRANSACTION REVRSAL--------------------");
    ...
    this.getICICIRequestResponseData3();
    ...
    this.mapping4Service.getProjectData(this.projectId).then((data) => {
      console.log("PRINT : MAPPING PROJECT DATA", data)
      ...
      console.log("PRINT : WEB SERVICE TYPE", this.webServiceType)
      this.mapping4Service.getServiceDetails(this.serviceId).then((data) => {
        ...
        this.mapping4Service.getUserDetails().then(async (data) => {

          console.log("PRINT: GET USER DETAILS", data)
          for (var i = 0; i < data.length; i++) {
            if (this.projectId == data[i].projectId) {

              console.log("PRINT: COMPARED PROJECT ID OUTPUT", data[i])
              this.mapping4Service.getUserDataByName(data[i].emailIdBusinessSpoc).then(async (data) => {
                console.log("PRINT : USER DATA BY NAME", data);

                this.username = data[0].username;
                this.organisation = data[0].organisation;
                this.clientCodeIPS = data[0].clientCodeIPS;//IPS CLIENT CODE
                ...
                if (this.clientCodeProfunds != undefined) {
                  this.clientCode = this.clientCodeProfunds;// PROFUND CLIENT CODE
                }

                console.log("PRINT : PROJECT ID", this.projectId);
                console.log("PRINT : PROJECT NAME", this.productName);
                console.log("PRINT : SERVICE NAME", this.serviceName);
                console.log("PRINT : USER NAME", this.username);
                console.log("PRINT : ORGANISATION", this.organisation);
                console.log("PRINT : POOL ACCOUNT NUMBER", this.poolAccountNumber);
                console.log("PRINT : IFSC CODE", this.ifscCode);
                console.log("PRINT : CLIENT CODE (PROFUND CLIENT CODE ASSIGNED)", this.clientCode);
                console.log("PRINT : IPS CLIENT CODE ", this.clientCodeIPS);
                console.log("PRINT : SERVICE URL", this.serviceUrl);

                await this.getClientFileService(this.projectId);
                await this.getClientFileService2(this.projectId);
                await this.getClientFileService3(this.projectId);
              })
            }
          }
        })
  ...
  requiredFieldCheck() {
    console.log("function called");
        for (var i = 0; i < this.requestData3.length; i++) {
          console.log("urgency name",this.requestData3);
          if (this.requestData3[i].directRowNo != "") {
            
            if (this.requestData3[i].urgencyName.includes("Mandatory") && (this.requestData3[i].sourceFieldName == "-")) {
  ...
  */
  getClientFileService3(projectId) {
    var type="uatReversalFile1";
    this.mapping4Service.getFileDataByProjectId(projectId,type).then(async (data) => {
      console.log(" PRINT : FETCH FILE DATA BY PROJECT ID ", data[3]);
      this.fileDataByProjectId3.push(data[3])
      await this.segregateSourceRequestData3(this.fileDataByProjectId3);

    })

  }
  segregateSourceRequestData3(sourceRequestArray) {
    console.log("PRINT : SOURCE SEGREGATE REQUEST DATA 3 ");
    console.log("PRINT : ARGS RECEIVED IN SEGREGATE 3 ", sourceRequestArray);
    var dataLength = sourceRequestArray.length;
    console.log("PRINT : ARGS RECEIVED LENGTH IN SEGREGATE 3  ", dataLength);

    for (var h = 0; h < dataLength; h++) {
      var operationsLength = sourceRequestArray[h].operations.length;
      console.log("SOURCE DATA CHECK", sourceRequestArray[h])
      for (var j = 0; j < operationsLength; j++) {
        this.sourceMethod3 = sourceRequestArray[h].operations[j].method;
        this.sourcePath3 = sourceRequestArray[h].operations[j].path;
        console.log(" Print : Source Path In Seggregate method 3 ", this.sourcePath3);
        this.sourceFields3 = sourceRequestArray[h].operations[j].fields;
        console.log(" Print : Source Fields In Seggregate method 3 ", this.sourceFields3, j);
        for (var k = 0; k < this.sourceFields3.length; k++) {
          console.log("Print : For Loop Started In Seggregate method 3 ", this.sourceFields3[k], k);
          this.extractedDataSource3 = this.nestedSegregationSource(this.sourceFields3[k]);
          var keyData = Object.keys(this.extractedDataSource3);
          var valueData = Object.values(this.extractedDataSource3)
          console.log(" Print : Keys Data Extraction In Seggregate method 3 ", keyData);
          console.log(" Print : Values Data Extraction In Seggregate method 3 ", valueData);

          for (var w = 0; w < keyData.length; w++) {
            this.combinedDataAfterExtractionSource3.push({
              key: keyData[w],
              value: valueData[w]
            });
          }
          console.log(" Print : Combined Data After Extraction In Seggregate method 3", this.combinedDataAfterExtractionSource2)

        }
        var inc = 1;
        for (var q = 0; q < this.combinedDataAfterExtractionSource3.length; q++) {
          var x = this.combinedDataAfterExtractionSource3[q].key;
          var splitted = x.split(".");
          console.log(" Print : Splitting Operation In Seggregate method 3 ", splitted)
          let splitLength = splitted.length
          if (splitted[splitLength - 1] == "type") {
            this.splitData3.push(splitted[splitLength - 2])
            this.requestDataSource3.push({
              id: inc,
              sourceName: this.sourcePath3,
              sfieldName: splitted[splitLength - 2],
              sdataType: this.combinedDataAfterExtractionSource3[q].value
            })
            inc++;
          }
        }
        console.log(" Print : Request Source Data In Seggregate method 3", this.requestDataSource3);
        console.log(" Print : Request Target Data In Seggregate method 3", this.requestDataICICI3);
        this.requestData3Before = this.sortSourceTargetData(this.requestDataSource3, this.requestDataICICI3);
        console.log(" Print : Request Sort Source Data In Seggregate method 3", this.requestDataSource3);
      }

    }
    this.requestData3 = this.requestData3Before;
    console.log(" Print : Request Data 3 ", this.requestData3);
  }
  getICICIRequestResponseData() {
    ...
    this.mapping4Service.getMappingSourceData(service).then((data) => {
      console.log(" Print: Request/Response Data 1 ", data);
  ...
  getICICIRequestResponseData2() {
    ...
    var service = "ECollection with Two Level Validation at Bank and Client’s End"
    this.mapping4Service.getMappingSourceData(service).then((data) => {
      console.log(" Print: Request/Response Data 2 ", data);
   ...
   * @description This function is called for the icici values
   */
  getICICIRequestResponseData3() {
    console.log(" Print: Fetch Request Response Data 3 ")
    var service = "ECollection with Two Level Validation at Bank and Client’s End";
    this.mapping4Service.getMappingSourceData(service).then((data) => {
      console.log(" Print : Response Of Mapping Source Data For Transaction reversal ", data);
      this.requestDataICICI3 = data[0].request3;
      console.log(" Print : Request ICICI Data 3  ", this.requestDataICICI3);
    })
  }
  /**
      * @author Sucheta
  ...
  */
  nestedSegregationSource(data) {
    console.log("CHECK DATA FOR NESTED SEGREGATION", data);
  ...
  sortSourceTargetData(clientData, iciciData) {
    ...
    for (var q = 0; q < largerLength; q++) {
      ...
      if (clientData[q] == undefined && clientData[q] == undefined && iciciData[q] != undefined) {
        ...
        console.log(" PUSH DATA => ", this.outputData, iciciData[q].tfieldName);
      }
    }
  ...
  */
  ...
  capture() {
    ...
    for(var i=0;i<this.requestData3.length;i++){
    if(this.requestData3[i].directRowNo !=""){ 
    exactCaptureRequest3.push(this.requestData3[i]);
    }
    }
    ...
    var tableData6 = [];
    for (var i = 0; i < exactCaptureRequest3.length; i++) {
    tableData6.push({
    'id': exactCaptureRequest3[i].id, 'sourceName': exactCaptureRequest3[i].sourceName, 'sfieldName': exactCaptureRequest3[i].sfieldName, 'sdataType': exactCaptureRequest3[i].sdataType,
    'targetName': exactCaptureRequest3[i].targetName, 'tfieldName': exactCaptureRequest3[i].tfieldName, 'tdataType': exactCaptureRequest3[i].tdataType, 'urgencyName': exactCaptureRequest3[i].urgencyName,
    'descriptionName': exactCaptureRequest3[i].descriptionName, 'directRowNo': exactCaptureRequest3[i].directRowNo, 'sourceFieldPath': exactCaptureRequest3[i].sourceFieldPath,
    'sourceFieldName': exactCaptureRequest3[i].sourceFieldName, 'tfieldNameMapping': exactCaptureRequest3[i].tfieldNameMapping, 'datatypeVerified': exactCaptureRequest3[i].datatypeVerified, 'backgroundColor': exactCaptureRequest3[i].backgroundColor
    })
    }
    doc.autoTable(cols, tableData6, {
    startY: doc.autoTable.previous.finalY + 60,
  ...
  deleteValueRequest3(data){
    console.log("PRINT : CALLING DELETE VALUE FUNCTION 3",data);
    for (var q = 0; q < this.requestData3.length; q++) {
      if (this.requestData3[q].id == data.id) {

        this.requestData3[q].directRowNo = "";
        this.requestData3[q].sourceFieldPath = "-";
        this.requestData3[q].sourceFieldName = "-";
        this.requestData3[q].datatypeVerified = "-";
        this.requestData3[q].tfieldNameMapping = "-"
        this.requestData3[q].backgroundColor = "true"
        console.log("final requestData = ", this.requestData[q])
      }

    }
    for(var q = 0;q<this.requestData3.length;q++){
      if(this.requestData3[q].directRowNo !=""){
  ...
  deleteValueRequest(data) {
    console.log("PRINT : CALLING DELETE VALUE FUNCTION 1",data);
...
*/
  ...
  */
  ...
  enterDirectRowNumberRequest2(data, event) {
    console.log("PRINT : CALLING enterDirectRowNumberRequest2() ", event);
    console.log("PRINT : MAPPING DATA ENTERED BY CLIENT ", data,data.sfieldName);
    if (data.sfieldName != undefined && event != undefined && event != null && event != "" && data.sfieldName != "") {
  ...
  deleteValueRequest2(data) {
    console.log("PRINT : CALLING DELETE VALUE FUNCTION 2",data);
    for (var q = 0; q < this.requestData2.length; q++) {
...
*/
  ...
  descriptionRequest3(a, data, iRequest) {

    this.modalService.open(iRequest, { size: 'sm' });
    this.descriptionICICIRequest3 = data.descriptionName
    this.fsizeICICIRequest3 = data.tfieldSize;
    this.dataTypeICICIRequest3 = data.tdataType
  }
  ...
  detailsClientRequest3(a, data, clientReq) {
    this.modalService.open(clientReq, { size: 'sm' });
    this.dataTypeClientRequest3 = data.sdataType
  }
   ...
   * @params : index, data, modal
   */
  detailsClientResponse3(a, data, clientResponse) {

    this.modalService.open(clientResponse, { size: 'sm' });
    this.dataTypeClientResponse3 = data.sdataType
...
checkVan(){
  ...
  */
  ...
  getFlattenStructure1(requestData, responseData, clientUrl) {
    ...
    //alert("finalEsqlObject object 1");
    console.log("finalEsqlObject object 1", JSON.stringify(finalEsqlObject1));
    this.mapping4Service.postESQL(finalEsqlObject1).then((dataEsql) => {
      console.log(" ESQL Response 1 ", dataEsql);
      //alert(" ESQL Response 1 ");
  ...
  */
  ...
  getFlattenStructureResponse1(requestData, responseData, clientUrl) {
    console.log(" Print : getFlattenStructureResponse1 ", requestData, responseData);
    ...
    console.log(" Print: DATA FOR ESQL RESPONSE ", this.dataForEsqlResponse);
    this.finalDataForEsql = {
      "sourceType": "JSON",
      "targetType": "DFDL",
    ...
    console.log(" finalEsqlObject2 object", JSON.stringify(finalEsqlObject2))
    this.mapping4Service.postESQL(finalEsqlObject2).then((dataEsql) => {
      console.log("ESQL Response =====> 2", dataEsql);
      //alert(" ESQL Response 2 ");
  ...
  async yamlCreation(requestD, responseD, clientUrl) {
    console.log(" YAML CREATION CALLED ", requestD, responseD);
    var flattenData =
    {
    ...
    console.log(" Print : combinedDataAfterExtractionSource ", this.combinedDataAfterExtractionSource);
    ...
    console.log(" Print : unflattenValue ", Object.assign(unflattenValue))

    var requestField = this.yamlGenRequest();
    var responseField = this.yamlGenResponse();
    console.log(" Print : Request fields ", requestField);
    console.log(" Print : Response fileds ", responseField);
    ...
    console.log(" Print : HARD CODED FLATTEN DATA ", flattenData);
    console.log(" Print : FINAL YAML ", finalYaml[0]);
    var yamlObject = {
      'params': finalYaml[0],
  ...
  yamlGenRequest() {
    console.log(" Print : YAML GEN REQUEST ");
  ...
  yamlGenResponse() {
    console.log(" Print : YAML GEN RESPONSE ")
  ...
  getFlattenStructure12() {
    console.log(" Print : getFlattenStructure12 ");
    this.dataForEsql2 = [];
    this.dataWithDirectRowNo2 = [];
    this.dataWithNoDirectRow2 = [];

    console.log(" Print : combinedDataAfterExtractionSource2  ", this.combinedDataAfterExtractionSource2)
    for (var i = 0; i < this.requestData2.length; i++) {
      if (this.requestData2[i].directRowNo !== "") {
    ...
    console.log("finalEsqlObject3 object ", JSON.stringify(finalEsqlObject3))

    this.mapping4Service.postESQL(finalEsqlObject3).then((dataEsql) => {
      //alert("ESQL Response 3 ")
      console.log("ESQL Response 3 ", dataEsql)
  ...
  */
  ...
  async getFlattenStructureResponse12() {
    console.log(" Print : getFlattenStructureResponse12 ");
    ...
    console.log("finalEsqlObject4 object", JSON.stringify(finalEsqlObject4));
    console.log("finalEsqlObject5 object", JSON.stringify(finalEsqlObject5));
    console.log("finalEsqlObject6 object", JSON.stringify(finalEsqlObject6));
    console.log("finalEsqlObject7 object transaction reversal ", JSON.stringify(finalEsqlObject7));




    await this.mapping4Service.postESQL(finalEsqlObject4).then(async (dataEsql) => {
      //alert("ESQL Response 4 ")
      console.log(" ESQL Response 4 ", dataEsql);

      this.toastr.success("Response for ESQL 4 :" + dataEsql.message);

      await this.mapping4Service.postESQL(finalEsqlObject5).then(async (dataEsql) => {
        //alert("ESQL Response 5 ")
        console.log("ESQL Response 5 ", dataEsql)
        this.toastr.success("Response for ESQL 5 :" + dataEsql.message);


        await this.mapping4Service.postESQL(finalEsqlObject6).then(async (dataEsql) => {
          //alert("ESQL Response 6")

          console.log("ESQL Response 6", dataEsql)

          this.toastr.success("Response for ESQL 6 :" + dataEsql.message);

          await this.mapping4Service.postESQL(finalEsqlObject7).then(async (dataEsql) => {
            console.log("ESQL Response 7", dataEsql);
            this.toastr.success("Response for ESQL 7 :" + dataEsql.message);
            ...
            let yaml_response3 = await this.yamlCreation3(this.requestData3, this.serviceUrl);
            console.log(" Print : YAML CREATION RESPONSE 3 ", yaml_response3);
  ...
  async yamlCreation3(requestD, clientUrl) {
    console.log("Request 3", requestD);
    var flattenData =
    {
    ...
    this.requestField3 = this.yamlGenRequest3();
    console.log("Request fields 3 =>>>>>>>", this.requestField3);
    ...
    let finalYaml2 = [{
      ...
      "operations": [{
        ...
        "path": this.sourcePath,
        "method": this.sourceMethod,
        fields: [this.requestField3['fields']],
        responses: []
      }]
    }]
    ...
    console.log("YAML Object For Transaction Reversal :", JSON.stringify(yamlObject2));
    this.mapping4Service.postYamlData(yamlObject2).then((yamlResponse) => {
      //alert("YAML Response 3-- Transaction Reversal")
      console.log("YAML Response 3 - Transaction Reversal ", yamlResponse)
      this.toastr.success("Transaction Reversal YAML 3 :" + yamlResponse.message);
  ...
  */
  ...
  async yamlCreation2(requestD, responseD, clientUrl) {
    console.log(" YAML CREATION 2 CALLED ");
    var flattenData =
    {
      "description": "",
      "title": "ecollection"+this.clientCode,
    ...
    this.responseField2 = this.yamlGenResponse2();
    console.log("Request fields 2", this.requestField2);
    console.log("Response fileds 2", this.responseField2)
      ...
      "operations": [
        {
          ...
          responses: [this.responseField2['responses']]
        }
      ]
    }]
  ...
  yamlGenRequest2() {
    console.log(" YAML REQUEST 2 CALLED ");
  ...
  yamlGenRequest3() {
    console.log(" YAML GEN REQUEST CALLED ");
    this.requestProperty3 = [];
    this.requestDataFiltered3 = [];
    this.requestLayout3 = [];
    this.requestPropertyNonArray3 = [];
    this.requestExtraction3 = [];
    this.requestPropNonArray33 = [];
    this.finalRequestWithoutArray3 = [];
    this.finalRequest123 = [];
    this.finalRequest223 = []
    ...
    for (var t = 0; t < this.requestData2.length; t++) {
      if (this.requestData2[t].directRowNo !== "") {
        ...
        this.requestDataFiltered3.push(this.requestData2[t])
      }

    }
    console.log("this.requestDataFiltered3 = ", this.requestDataFiltered3)
    //step 2 - check if any array from request data is present in flattened array

    // this.combinedDataAfterExtractionSource2

    for (var y = 0; y < this.requestDataFiltered3.length; y++) {

      for (var u = 0; u < this.combinedDataAfterExtractionSource2.length; u++) {
        if (this.combinedDataAfterExtractionSource2[u].key.includes(this.requestDataFiltered3[y].sourceFieldName)) {
      ...
      // console.log(" this.requestLayout3 = ", this.requestLayout3)
    }

    //step 3  - segregate the array values having a corresponding flattened array type in a new json

    for (var d = 0; d < this.requestDataFiltered3.length; d++) {

      for (var s = 0; s < this.combinedDataAfterExtractionSource2.length; s++) {
        if (this.combinedDataAfterExtractionSource2[s].key.includes("." + this.requestDataFiltered3[d].sourceFieldName + ".") && ((this.requestDataFiltered3[d].datatypeVerified) !== "array")) {
          // console.log("this.combinedDataAfterExtractionSource2[u].key = ", this.combinedDataAfterExtractionSource2[s].key)

          // console.log("this.requestDataFiltered3[y].sourceFieldName = ", this.requestDataFiltered3[d].sourceFieldName)
          for (var f = 0; f < this.combinedDataAfterExtractionSource2.length; f++) {
            ...
            if (this.combinedDataAfterExtractionSource2[f].key.includes(this.combinedDataAfterExtractionSource2[s].key) && (this.combinedDataAfterExtractionSource2[f].value !== "array")) {
              ...
              this.requestProperty3.push({
                key: this.combinedDataAfterExtractionSource2[s].key, value: {



                  [this.requestDataFiltered3[d].sourceFieldName]: {
                    "type": this.requestDataFiltered3[d].datatypeVerified,
      ...
      console.log(" this.requestProperty3 = ", this.requestProperty3)
    }
    //step 4 - Request non-array values from request data into a new json

    for (var g = 0; g < this.requestDataFiltered3.length; g++) {

      for (var h = 0; h < this.combinedDataAfterExtractionSource2.length; h++) {

        if (arrayFlag == true) {

          if (!((this.combinedDataAfterExtractionSource2[h].key).includes(this.requestDataFiltered3[g].sourceFieldName)) && ((this.requestDataFiltered3[g].datatypeVerified) !== "array")) {
            // console.log("Inside if od step 4")
            // console.log("this.combinedDataAfterExtractionSource2[h].key = ", this.combinedDataAfterExtractionSource2[h].key)
            ...
            for (var j = 0; j < this.combinedDataAfterExtractionSource2.length; j++) {
              ...
              if (this.combinedDataAfterExtractionSource2[j].key.includes(this.combinedDataAfterExtractionSource2[h].key) && ((this.combinedDataAfterExtractionSource2[j].value) !== "array")) {
                ...
                this.requestPropertyNonArray3.push({
                  key: this.requestDataFiltered3[g].sourceFieldName, value:
                  {
                    [this.requestDataFiltered3[g].sourceFieldName]: {
                      "type": this.requestDataFiltered3[g].datatypeVerified,
    ...
    if (arrayFlag === true) {
      ...
      for (var l = 0; l < this.requestPropertyNonArray3.length; l++) {
        var flag = false;
        for (var z = 0; z < this.requestProperty3.length; z++) {

          var value1 = Object.keys(this.requestPropertyNonArray3[l].value)
          var value2 = Object.keys(this.requestProperty3[z].value)
        ...
        if (flag === false) {
          this.requestPropNonArray33.push(this.requestPropertyNonArray3[l])
        }
      }
    }
    else {
      this.requestPropNonArray33 = (this.requestPropertyNonArray3)
    }

    console.log("this.requestPropNonArray33 = ", this.requestPropNonArray33)
    ...
    for (var x = 0; x < this.requestProperty3.length; x++) {

      if (this.requestLayout3.length !== 0) {

        for (var c = 0; c < this.requestLayout3.length; c++) {

          if (this.requestProperty3[x].key.includes(this.requestLayout3[c].key)) {
            console.log("Inside last if  = ")
            console.log("Object.keys(this.requestProperty3[x].value)[0] = ", Object.keys(this.requestProperty3[x].value)[0]);
            console.log("Object.values(this.requestProperty3[x].value)[0] = ", Object.values(this.requestProperty3[x].value)[0]);
            console.log("this.requestLayout3[c].key = ", this.requestLayout3[c].key)
            this.finalRequest123 = this.requestLayout3[c].value
            console.log("finalRequest123", this.finalRequest123)
            this.finalRequest123[this.requestLayout3[c].key].items.properties[Object.keys(this.requestProperty3[x].value)[0]] = Object.values(this.requestProperty3[x].value)[0]
          }
    ...
    for (var v = 0; v < this.requestPropNonArray33.length; v++) {
      this.finalRequestWithoutArray3.push(this.requestPropNonArray33[v].value)

    }

    console.log("Final requestLayout3 = ", this.requestLayout3)
    console.log("this.finalRequestWithoutArray3 = ", this.finalRequestWithoutArray3);


    var fields = {};

    console.log("this.finalRequest123 = ", this.finalRequest123)
    console.log("this.finalRequestWithoutArray3 = ", this.finalRequestWithoutArray3)


    if (this.requestLayout3.length !== 0) {

      this.finalRequest223.push(this.finalRequest123)
    }

    this.finalRequestWithoutArray3.forEach((item) => {
      console.log("finalRequestWithoutArray3 , Item = ", item)
      var key1 = Object.keys(item)
      this.finalRequest223.push(item)
    })
    console.log("this.finalRequest223 = ", this.finalRequest223);

    if (arrayFlag == true) {
      fields['fields'] = this.finalRequest223
    }
    else if (arrayFlag == false) {
      var result = Object.assign({}, ...this.finalRequest223);

      console.log("result of Object.assign = ", result)

      fields["fields"] = result;
    }
    console.log("Fields Request in Yaml Generate 3 ", fields)

    return fields;
  }

  yamlGenResponse2() {
    console.log(" YAML GEN RESPONSE 2 CALLED ");
  ...
  /**
     ...
     * @description : Mapping for Request
     */
  enterDirectRowNumberRequest3(data, event) {
    console.log("PRINT : CALLING enterDirectRowNumberRequest3() ", event);
    console.log("PRINT : MAPPING DATA ENTERED BY CLIENT ", data,data.sfieldName);
    try{
    if (data.sfieldName != undefined && event != undefined && event != null && event != "") {
      for (var q = 0; q < this.requestData2.length; q++) {
        if (this.requestData3[q].id == data.id && event != undefined && event != null) {
          if (this.requestData3[event - 1].tdataType != null) {
          if (this.requestData3[event - 1].sdataType != "array") {
            if (this.requestData3[event - 1].sdataType != "object") {
              this.requestData3[q].directRowNo = event;
              this.requestData3[q].sourceFieldPath = this.requestData3[event - 1].sourceName;
              this.requestData3[q].sourceFieldName = this.requestData3[event - 1].sfieldName;
              this.requestData3[q].datatypeVerified = this.requestData3[event - 1].sdataType;
              this.requestData3[q].tfieldNameMapping = this.requestData3[q].tfieldName;
              var targetDatatype = (this.requestData3[q].tdataType);
              var sourceDatatype = (this.requestData3[event - 1].sdataType);
              console.log("PRINT :REQUEST DATA ", this.requestData3[q]);
            } else {
